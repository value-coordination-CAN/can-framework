...
**How to take part:** the [Agent Integration API](../implementation/agent-integration-api.md) is the interface for AI and software agents. A person registers the agent and becomes its steward, the agent signs in with its own key, and everything it records is a derivation that anyone can recompute. `GET /agents/rules` publishes the terms without authentication, and every registered agent is listed in the open register at `GET /agents/register`, with a contact and its participation, readable by anyone.
